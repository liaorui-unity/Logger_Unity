...
namespace Sailfish.Log
{
    public class LogUploader
    {
        ...
        private bool SocketClient()
        {
            ...
            IPAddress ip = IPAddress.Parse(Info.debugTypeLAN ? Info.uploadLAN : Info.uploadNET);//获取服务器IP地址
            IPEndPoint point = new IPEndPoint(ip, Info.uploadPort);//获取端口
            try
            {
            ...
            {
                Info.LogError("Log与服务器链接失败_" + ex);
                return false;
            }

            Info.Log("与服务器链接成功！:"+ ip);
        ...
        {
           // Info.isUploadDebug = true;
            try
            {
                using (FileStream fsRead = new FileStream(Info.logFileSavePath, FileMode.OpenOrCreate, FileAccess.Read))
                {
                    //1. 第一步：发送一个文件，表示文件名和长度，让客户端知道后续要接收几个包来重新组织成一个文件
                    await Task.Run(() => { SendFileInfo(Info.logFileSavePath, fsRead); });
        ...
        {
            string fileName = Info.debugFileDate;
        
            string folder = string.Format("{0}/{1}/{2}", Info.debugName, Info.debugIp, Info.debugDate);
        ...
        {
             Info.Log(string.Format("Process.recv -> [  {0}%  ]", (proc * 100).ToString()));
        }
    }
}
