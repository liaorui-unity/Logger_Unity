...
namespace NET
{
    ...
    public class SyncPacketPools
    {
        private static Dictionary<System.Type, ushort> m_packet_UshortIDs = new Dictionary<System.Type, ushort>();
        /// <summary>
        /// 反射出的协议信息
        /// </summary>
        private static Dictionary<ushort, SyncPacketInfo> m_packet_infos = new Dictionary<ushort, SyncPacketInfo>();
        /// <summary>
        /// 对象池
        /// </summary>
        private static Dictionary<ushort, List<SyncMsg>> m_packet_pools = new Dictionary<ushort, List<SyncMsg>>();
#if DEBUG
        private static Dictionary<ushort, long> m_new_count = new Dictionary<ushort, long>();
#endif
        public static SyncMsg Get(ushort id)
        {
            SyncMsg packet = null;
            ...
            while (packet == null)
            {
                ...
                {
                    SyncPacketInfo packet_info = GetPacketInfo(id);
                    if (packet_info == null)
                    {
                        Debuger.LogError("PacketPools - 没有找到协议对应的类信息，请在类{0}添加特性PacketAttribute");
                        return null;
                    }
                    packet = (SyncMsg)Activator.CreateInstance(packet_info.Type, true);
                    if (packet == null)
                    {
        ...
        public static T Get<T>() where T : SyncMsg
        {
            SyncMsg packet = null;
            ushort id = 0;
        ...
        #region 协议信息
        public static void AddPacketInfo(SyncPacketInfo info)
        {
        ...
        public static SyncPacketInfo GetPacketInfo(ushort id)
        {
            SyncPacketInfo info = null;
        ...
        {
            System.Reflection.Assembly.GetAssembly(typeof(SyncMsg));

            float start_time = Time.realtimeSinceStartup;
            //外部程序集
            List<System.Type> types = fs.AttributeUtils.FindType<SyncMsg>(assembly, false);
            if (types != null)
            {
                foreach (System.Type type in types)
                {
                    SyncPacketAttribute attr = fs.AttributeUtils.GetClassAttribute<SyncPacketAttribute>(type);
                    if (attr == null) continue;
                    AddPacketInfo(new SyncPacketInfo() { Id = GetID(type), Type = type });
                }
            }
        ...
        static ushort GetID(System.Type type)
        {
            ...
            AddPacketInfo(new SyncPacketInfo() { Id = id, Type = type });
            return id;
        }
