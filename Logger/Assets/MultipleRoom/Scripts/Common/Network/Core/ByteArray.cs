...
namespace NET
{
    ...
    {
        private byte[] m_Buffer;            //缓冲区数组
        private int m_BufferLen;            //缓冲区长度
        private int m_MaxBufferLen;         //缓冲区最大长度
        private int m_Head;                 //头
        private int m_Tail;                 //尾
        ...
        public bool Resize(int size)
        {
            ...
            {
                Debuger.LogError("ByteArray::Resize 缓冲区溢出:" + newBufferLen);
        ...
        /// 写入bytes
        /// </summary>
        /// <param name="buf">源数据</param>
        /// <param name="start_index">开始位置</param>
        /// <param name="len">长度</param>
        /// <returns></returns>
        public int Write(byte[] buf, int start_index, int len)
        {
            //					//
            //     T  H			//    H   T			LEN=10
            ...
            if (m_Head <= m_Tail)
            {
                if (m_Head == 0)
                {
                    ...
                    Array.Copy(buf, start_index, m_Buffer, m_Tail, len);
                }
                else
                {
                    ...
                    {
                        Array.Copy(buf, start_index, m_Buffer, m_Tail, nFree);
                        Array.Copy(buf, nFree + start_index, m_Buffer, 0, len - nFree);
                    }
                }
            }
            else
